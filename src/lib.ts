...
export const PRIVACY_PATTERNS: { name: string; regex: RegExp; replacement: string }[] = [
...
export function generateTitle(problem: string, errorMessage?: string): string {
  if (errorMessage) {
...
export function validateContribution(input: ContributeInput): string[] {
  const issues: string[] = [];
